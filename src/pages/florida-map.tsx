...
import { CAMPUSES, FLAME_GOLD, type Campus } from "../lib/campus-data";
import {
  FLORIDA_MAP_ASSET,
  LEGEND_HITS,
  MAP_HEIGHT,
...
function campusById(id: string): Campus | undefined {
  ...
  return (
    <group>
      {MAP_HOTSPOTS.map((s, i) => {
        ...
        const hot = hoveredId === s.campusId;
        return (
          <mesh
            key={`${s.campusId}-${i}`}
            position={[x, 0.06, z]}
